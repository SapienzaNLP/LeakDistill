...
class AMRBartTokenizer(BartTokenizer):
    ...
    def __init__(self, *args, use_pointer_tokens=False, collapse_name_ops=False, **kwargs):
        ...
        self.direction = 'amr'
        self.vocab_path = 'data/vocab/amr/'
    ...
    def pre_tokenize(self, text):
        ...
        for tok_span in text.lstrip().split(' '):
            ...
            if len(recats) == 2 and recats[0] in self.recategorizations and (
                    self.convert_tokens_to_ids('_' + recats[1]) != self.unk_token_id):
    ...
    def tokenize_amr(self, graph):
        ...
        for i, (backr, tokk) in enumerate(zip(backreferences, linearized_nodes)):
            is_in_enc = (self.unk_token != self.INIT + tokk and self.convert_tokens_to_ids(
                self.INIT + tokk) != self.unk_token_id) or (tokk in self.special_tokens_list)
            ...
            if tokk.startswith('"') and tokk.endswith('"'):
                ...
                if is_in_enc:
                    ...
                    rel = tokk[:-3]
                    if self.unk_token != self.INIT + rel and self.convert_tokens_to_ids(
                            self.INIT + rel) != self.unk_token_id:
                        bpe_toks = [self.INIT + rel, '-of']
                    else:
                        bpe_toks = [self.INIT + ':'] + [tokkk if id_tokk == 0 else tokkk for id_tokk, tokkk in
                                                        enumerate(self._tok_bpe(rel[1:]))] + ['-of']
                elif is_rel:
                    bpe_toks = [self.INIT + ':'] + [tokkk if id_tokk == 0 else tokkk for id_tokk, tokkk in
                                                    enumerate(self._tok_bpe(tokk[1:]))]
                else:
                    raise
...
class PENMANBartTokenizer(AMRBartTokenizer):
    ...
    def tokenize_amr(self, graph):
        if self.raw_graph:
            ...
            bpe_tokens = self._tokenize(linearized)[:1022]
            bpe_token_ids = [self.convert_tokens_to_ids(b) for b in bpe_tokens]
